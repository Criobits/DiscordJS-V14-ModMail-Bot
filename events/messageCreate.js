            ...
            if (message.guild) {
                if (message.content.startsWith('!')) {
                    ...
                    if (customCmdData.length > 0) {
                        ...
                        const embed = new EmbedBuilder().setTitle(cmd.embedTitle).setDescription(cmd.embedDescription).setColor('Gold').setFooter(footer).setTimestamp();
                ...
                if (isReply || isAnonReply) {
                    ...
                    try {
                        if (content) {
                            let replyContent;
                            if (isAnonReply) {
                                replyContent = `**Lo staff risponde:** ${content}`;
                            } else {
                                replyContent = `**${message.author.displayName} risponde:** ${content}`;
                            }
